package racingcar;

public class Messages {
    public static final String ASK_CAR_NAMES = "경주할 자동차 이름을 입력하세요.(이름은 쉼표(,) 기준으로 구분)";
    public static final String ASK_TURN_COUNTS = "시도할 횟수는 몇 회인가요?";
    public static final String RUN_RESULT = "실행 결과";
    public static final String FINAL_CHAMPION = "최종 우승자 : ";

    public static final String ERROR_EMPTY = "빈 값은 허용되지 않습니다.";
    public static final String ERROR_OVER_NAME = "이름은 5자 이하로 설정해야합니다.";
    public static final String ERROR_NAME_DUPLICATE = "이름이 중복되었습니다.";
    public static final String ERROR_IS_NOT_NUMBER = "1이상의 자연수만 입력가능합니다.";
    public static final String ERROR_CAR_NAME_EMPTY = "자동차 이름은 공백일 수 없습니다.";
}
